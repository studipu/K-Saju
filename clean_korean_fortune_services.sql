-- Clean Korean Fortune Telling Services Data
...
-- STEP 4: Add Korean fortune telling services
...
CASE WHEN current_setting('app.language', true) = 'en' THEN 'Mystic Tarot Salon' ELSE '미스틱 타로 살롱' END, 
CASE WHEN current_setting('app.language', true) = 'en' THEN 'Unlock your future with mystical tarot cards' ELSE '신비로운 타로카드로 미래를 읽어드립니다' END,
CASE WHEN current_setting('app.language', true) = 'en' THEN 'Premium tarot consultation featuring authentic French-imported tarot cards and a tarot master with 30 years of experience. Find clear answers to all your questions about love, career, and relationships through the wisdom of tarot cards.' ELSE '프랑스에서 직수입한 정통 타로카드와 30년 경력의 타로마스터가 함께하는 프리미엄 타로 상담소입니다. 연애, 진로, 인간관계 등 궁금한 모든 것을 타로카드를 통해 명확히 해답을 찾아드립니다.' END,
...
SELECT 'All unrelated location data deleted and Korean fortune services added successfully!' as status;
